                            ...
                            if(preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}+$/', $horaire)){
                                if(preg_match('/^[1-9]{1}[0-5]{0,1}+$/', $place)){
                                    if($choix != "Intérieur" OR $choix != "Terrasse"){
                                        ...
                                        header('location:reservation.php?book_err=success');
                                        
                                    }else header('location:reservation.php?book_err=choice_validate');
                                }else header('location:reservation.php?book_err=cutlery_validate');
                            }else header('location:reservation.php?book_err=date_validate');
                        }else header('location:reservation.php?book_err=email_validate');
                    }else header('location:reservation.php?book_err=tel_validate');
                }else header('location:reservation.php?book_err=ncname_validate');
            }else header('location:reservation.php?book_err=name_validate');
        }else header('location:reservation.php?book_err=all_inputs');
    }else header('location:reservation.php?book_err=button');
